...
TEMPLATE ENGINE
 - something like html but still different  h=blub,  html(body(h('hello')))
 - ohe you can just do anything every where <% count++; return 'blub'; %>
 - invented a whole new own language paradigma wihtout any grammar restrictions, checks or error messages  bind="x in set | uppercase | currency"
 - and a two way dom<->object binding ;  delete o.items; o.items=fetch1milltionItemFromDB(); // wow look first evry thing is gone and then...
 - an awesome peg precompiler to comfortably drink some coffee while you wait to become faster once the browser finally understand our awesome invention

EVENTHANDLING
 - bind to the target to get the message, but dn't forget to unbind, eh bind when it is inserted later an oh we do also broad cast some things.
 - a declerative hash map with string in our custom syntax to register by magic a listener to a dom item, which will then call as function found at a name with the same in CamelCase but with an _ and beginning with 'On'.
 - so you can send the event or call that function directly well, then there are no listeners and eh.

MODEL VIEW CONTROLLER
 - so you have this glue controller, and then there are views, but some views have state to so there also view models
 - well the json from the backen is magically merged right in to our own model object structures where each property is an intelligent structure you can register on

BACKEND CALLS
 - are asynchornic of course (just like the event paradigm) but you can use our cool promises, be care full to pass them around but hey it allows a free mixing with synchronic code.
 - but for the simple cases we have adapter so you don't need to call rest services directly, we do this http calling thing for you

OTHER DID NOT...
 - reactive: just listen to this single value stream, and then merge this with this other stream, add some observers and then produce new values which you can then split into new streams or merge them togeter again.
 - virtual dom diff: just paint everything for nothing and we make a diff thus ignor that to apply only just that. Well no its not a real dom
 - we extend this html with import causing many requests but someday this will disappear

 Well you have state in the dom, but that inconvenient so we have state data in view objects as well and in the modell object. And then we get data from the backend and merge that into our model state and while that happens the user enters something changing that state,
 so we notify the view with their states and render them changing the dom state wich in turn can alter the outcome of calculations or trigger new events, then event handlers are used to create some asynchronic calls depending on the state where we have some dirty flags
 and eh... well because it is such a mess we split this up in two unrelated parts and then programm some untestable glue between it so 
 well this belong in the view, not the the scope as a service. The controller is in control and then the model checks the data, but not if its comming from the back end of course.
 but its very convenient to seperate every thing in multiple files. no you can not use them without any controlle. So we thought we putt that together with the view.
 We got this action abstraction. If its a button, an icom or menu it always is transfromed to an action ant then you can register your event handler there, or pass the action object around. Endless possibilities.
